...
class ConvNeXt_features(nn.Module):
    ...
    def __init__(self, 
        ...
        for i in range(num_stages):
            ...
            cur += depths[i]
        if use_transformer:
            stage = nn.ModuleList([TransformerBlock(dim=dims[-1], drop_path=dp_rates[cur + j], layer_scale_init_value=layer_scale_init_value, ten_net=ten_net) for j in range(depths[-1])])
            self.stages.append(stage)
            cur += depths[-1]
...
class ConvNeXt3d_features(nn.Module):
    r""" ConvNeXt
        A PyTorch impl of : `A ConvNet for the 2020s`  -
          https://arxiv.org/pdf/2201.03545.pdf
    Args:
...
def convnext_micro(pretrained=False, ten_net=0, in_chan=3, use_transformer=False, **kwargs):
    model = ConvNeXt(depths=[3, 3, 6, 3], dims=[64, 128, 256, 512], ten_net=ten_net, in_chans=in_chan, use_transformer=use_transformer, **kwargs)
    if pretrained:
        print('---------------')
...
def convnext_small(pretrained=False, ten_net=0, in_chan=3, use_transformer=False, **kwargs):
    model = ConvNeXt(depths=[3, 3, 27, 3], dims=[96, 192, 384, 768], ten_net=ten_net, in_chans=in_chan, use_transformer=use_transformer, **kwargs)
    if pretrained:
...
def convnext_base(pretrained=False, ten_net=0, in_chan=3, use_transformer=False, in_22k=False, **kwargs):
    model = ConvNeXt(depths=[3, 3, 27, 3], dims=[128, 256, 512, 1024], ten_net=ten_net, in_chans=in_chan, use_transformer=use_transformer, **kwargs)
    if pretrained:
...
#@register_model
def convnext_tiny_features(pretrained=False, ten_net=0, in_chan=1, use_transformer=False, **kwargs):
    model = ConvNeXt_features(depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], ten_net=ten_net, in_chans=in_chan, use_transformer=use_transformer, **kwargs)
    if pretrained:
        url = model_urls['convnext_tiny_1k']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", check_hash=True)
        weights_dict = checkpoint["model"]
        weights_dict.pop('head.weight')
        weights_dict.pop('head.bias')
...
def load_params2d_features(model2d, pretrained_path, ten_net, use_transformer, size='small'):
    if size == 'tiny':
        modelname = 'convnextransformer' if use_transformer else 'convnext'
    elif size == 'small':
        modelname = 'convnextransformerSmall' if use_transformer else 'convnextSmall'
    ...
    path = os.path.join(pretrained_path, modelname, tennetname, name)
    print(path)
    weights_dict = torch.load(path)['model']
    weights_dict.pop('head.weight')
    weights_dict.pop('head.bias')
    model2d.load_state_dict(weights_dict, strict=False)
    return model2d
...
def load_params3Dfromparams3D_features(model, pretrained_path, ten_net, use_transformer, size='small', datasize=256, pretrained_mode='multitaskECCV'):
    if size == 'tiny':
        modelname = 'convnextransformer' if use_transformer else 'convnext'
    elif size == 'small':
        modelname = 'convnextransformerSmall' if use_transformer else 'convnextSmall'
    ...
    checkpoint = torch.load(path)
    model_sd = model.state_dict()
    model_sd.pop('metadata_embedding.weight')
    model_sd.pop('metadata_embedding.bias')
    model_sd.pop('metadata_prehead.weight')
    model_sd.pop('pre_head.weight')
    model_sd.pop('pre_head.bias')
    model_sd.pop('norm.weight')
    model_sd.pop('norm.bias')
    ...
    if pretrained:
        ...
        if pretrained_mode == 'imagenet':
            if in_chan == 3:
                ...
                model2d = convnext_micro(pretrained=False, ten_net=ten_net, in_chan=in_chan, use_transformer=use_transformer, **kwargs)
    ...
    if pretrained:
        ...
        if pretrained_mode == 'imagenet':
            if in_chan == 3:
                ...
                model2d = convnext_small(pretrained=False, ten_net=ten_net, in_chan=in_chan, use_transformer=use_transformer, **kwargs)
    ...
    if pretrained:
        ...
        if pretrained_mode == 'imagenet':
            if in_chan == 3:
                ...
                model2d = convnext_base(pretrained=False, ten_net=ten_net, in_chan=in_chan, use_transformer=use_transformer, **kwargs)
...
def convnext_tiny_3d_features(pretrained=False, 
                              added_dim=2, init_mode='full', 
                              ten_net=0, in_chan=1, use_transformer=False, 
                              pretrained_path=None, pretrained_mode='multitaskECCV', drop_path=0.1, datasize=256, **kwargs):
    assert added_dim in [2]#[0,1,2] # Symbolic for (D)epth, (H)eight, (W)idth
    pretrained=True
    pretrained_path="/home/lisadesanti/DeepLearning/Lisa/pretrained_backbones/convnext3d_pretrained/saved_models/"
    model3d = ConvNeXt3d_features(num_classes=2, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], ten_net=ten_net, in_chans=in_chan, use_transformer=use_transformer, drop_path_rate=drop_path, **kwargs)
    ...
    if pretrained:
        ...
        if pretrained_mode == 'imagenet':
            if in_chan == 3:
                model2d = convnext_tiny_features(pretrained=pretrained, ten_net=ten_net, in_chan=in_chan, use_transformer=use_transformer, **kwargs)
            else:
                model2d = convnext_tiny_features(pretrained=False, ten_net=ten_net, in_chan=in_chan, use_transformer=use_transformer, **kwargs)
                model2d = load_params2d_features(model2d, pretrained_path, ten_net, use_transformer, size='tiny')
            model3d = load_params3Dfromparams2D(model2d, model3d, init_mode, init_kind, added_dim)
        else:
            load_params3Dfromparams3D_features(model3d, pretrained_path, ten_net, use_transformer, size='tiny', datasize=datasize, pretrained_mode=pretrained_mode)
    return model3d
